...
export default async function ReviewLayout({ children }: { children: ReactNode }) {
  // Read the session without redirecting (this layout also wraps /review/login, accept-invite, and
  // reset-password, which must stay reachable while signed out). The console nav below renders ONLY
  // when authenticated, so it does not show on the logged-out login page; this is render-only and
  // does NOT enforce auth (each page still gates itself). The Settings link is admin-only.
